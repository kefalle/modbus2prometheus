...
Документ сверён с состоянием ветки `fix/write-during-reconnect` на 2026-09-01.
Первая итерация безопасного рефакторинга завершена; текущий backlog и порядок
следующих изменений описаны в `docs/REFACTORING_PLAN.md`.
...
Команды компилируют все четыре пакета. Исполняющие тесты покрывают lifecycle,
reconnect и write-during-reconnect Controller, snapshot isolation, HTTP write
contract и Bearer auth, server timeouts, регистрацию Telegram-команд и команды
`sens`/`sust`. Форматирование проверяется командой `gofmt -l .`, которая не
должна выводить файлов.
...
В Docker Compose конфигурация хоста монтируется в `/app/config.yaml`.
Приложение и nginx используют `network_mode: host`: приложение слушает host
port `9101`, nginx — host port `80`. Nginx удаляет префикс
`/modbus2prometheus/` и передаёт запрос на `127.0.0.1:9101`; остальные routes
ведут к Grafana, VictoriaMetrics, Node-RED и Zigbee2MQTT на том же host.
...
Чтение и запись сериализованы mutex Controller. Если polling закрыл соединение
после ошибки чтения, запись сначала повторно открывает Modbus-клиент; этот
...
цифры и underscore. Код регистрирует имена без ведущего `/`; ошибка регистрации
меню возвращается в `main` как ошибка запуска.

## 9. Текущее состояние

### Завершённая безопасная основа
...
3. Polling и write сериализованы; запись после закрытия соединения сначала
   выполняет reconnect.
4. Controller использует минимальный Modbus-интерфейс, fake client и отдельный
   VictoriaMetrics `Set` на экземпляр.
5. Telegram не записывает текст, `NaN` или `Inf`; Node-RED transport/status/JSON
   errors обрабатываются без panic, а pressure выводится из правильного поля.
6. Presentation adapters читают immutable `TagSnapshot`; race-тест одновременно
   выполняет polling и snapshot reads.
7. HTTP write имеет явные статусы, JSON error body, лимит 1 MiB, строгие поля,
   server timeouts и опциональную Bearer-аутентификацию.
8. Ошибки библиотек возвращаются вызывающему коду; только `main` определяет
   exit policy.

Исходный `docs/REFACTORING_PLAN.md` выполнен. Его задачи заменены новым планом,
а завершённые гарантии перечислены выше и защищены regression-тестами.

### Ограничения до выполнения нового плана

- Пустой `http.writeBearerToken` сохраняет legacy-режим без аутентификации.
  Такой режим допустим только в доверенной сети. Новый план отключает write API
  по умолчанию и требует Bearer token при явном включении.
- `WriteTagByName` приводит `float64` к `uint16`/`float32` без проверки дробной
  части, переполнения и диапазона оборудования. До добавления обязательных
  `min`/`max`/`step` оператор отвечает за безопасное входное значение.
- Состояние Telegram-диалога общее для всех владельцев и чатов, а
  `CallbackQuery` отдельно по allowlist не проверяется.
- Telegram запускается даже при пустом token и пока не является опциональным.
- YAML decoder принимает неизвестные поля; не проверяются повторные теги,
  конфликтующие операции, metric names и положительные duration. Открытый
  YAML-файл не закрывается.

### Технический долг

- Опечатка `TagsHahdler`, непоследовательные `ust`/`sust` и mixed-language
  имена усложняют навигацию. Внешние имена Telegram-команд менять нельзя.
...
### Этапы 0–1. Безопасный lifecycle и тестовый шов — выполнены

- Lifecycle race исправлен, `os.Exit` удалён из Controller.
- Polling, reconnect, cancel, snapshot и HTTP write покрыты тестами.
- Запись во время reconnect повторно открывает клиент и не пересекается с
  Modbus read/write/close.
- Некорректный Telegram-ввод не достигает Modbus write.
- Ошибки Node-RED и Telegram API возвращаются без panic/process exit.
- Добавлены строгий HTTP contract, body limit, server timeouts и optional
  Bearer auth.

### Этап 2. Валидировать домен и конфигурацию — запланирован

- Покрыть успешные `float32` read/write до изменения модели.
- Сделать операции строгим типом и отвергать неизвестные, повторные и
  конфликтующие флаги.
- Проверять YAML, URL, уникальность имён и адресов, metric syntax и duration.
- Ввести обязательные `min`/`max`/`step` для writable-тегов и централизовать
  проверку значения до преобразования типа.
- Сделать write API выключенным по умолчанию и требовать Bearer token при его
  включении.
- Сделать Telegram опциональным и проверять его секцию только при включении.

### Этап 3. Изолировать adapters и Telegram conversations — запланирован

- Оставить правила чтения/записи в Controller как application service, а HTTP
  и Telegram перевести на минимальные интерфейсы.
- Возвращать typed errors для некорректного, выходящего за диапазон и
  не соответствующего шагу значения.
- Хранить Telegram conversation state по паре `(chatID, userID)`.
- Проверять allowlist одинаково для сообщений и callback queries.

### Этап 4. Наблюдаемость и release contract — запланирован

- Добавить `/healthz` и `/readyz`; readiness зависит от успешного Modbus read.
- Добавить счётчики reconnect attempts, write requests и write failures.
- Выпускать отдельные Linux artifacts для подтверждённых ARM targets.

### Этап 5. Локальная чистка типов и имён — запланирован

- Исправить внутреннюю опечатку `TagsHahdler` без изменения URL.
- Удалить неиспользуемые поля и закомментированный код.
- Заменить mutable `interface{}` в модели тега на явный тип значения.
- Перемещение всего проекта в `cmd/` и `internal/` не входит в текущий план:
  оно не нужно для закрытия перечисленных рисков и может выполняться отдельно.

Пошаговая последовательность, exact files, тесты и migration notes находятся в
`docs/REFACTORING_PLAN.md`.

## 14. Входные решения для нового плана

До начала соответствующих задач нужно зафиксировать внешние данные, которые
нельзя безопасно вывести из кода:

1. Утверждённые `min`, `max` и `step` для шести writable-регистров из карты
   конкретного оборудования. Значения нельзя угадывать по текущим показаниям.
2. Доступность HTTP после миграции: план сохраняет listen address `:9101`,
   отключает write API по умолчанию и требует Bearer token при включении.
3. Telegram должен поддерживать одновременные независимые диалоги всех
   пользователей из allowlist; ключ состояния — `(chatID, userID)`.
4. Release-задача в плане предполагает ARMv7 и ARM64. Перед её выполнением
   владелец deployment должен подтвердить, что ARMv6 не требуется.

Эти решения не блокируют добавление regression-тестов, строгого YAML parsing и
внутренний рефакторинг. Диапазоны блокируют только включение обязательной
domain validation для production-конфигурации, а ARM policy — только изменение
release workflow.
