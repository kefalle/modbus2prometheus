...
6. HTTP write имеет явные статусы, JSON error body, лимит 1 MiB, строгие поля и
   server timeouts.
7. Ошибки разбора Modbus-операций, инициализации Telegram и callback-запросов
   возвращаются вызывающему коду; только `main` определяет exit policy.
...
2. Telegram запускается даже при пустом token и пока не является опциональным;
   ошибка инициализации корректно возвращается в `main` и останавливает запуск.
